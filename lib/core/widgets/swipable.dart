...
class Swipable extends StatefulWidget {
  ...
  ///Hook triggered when the swipe ended
  final void Function()? afterSwipe;
  ...
  /// @param shouldResetPosition [bool]
  /// To enable or disable the reset position
  /// defaults to false
  // final bool shouldResetPosition;
...
class SwipableState extends State<Swipable> {
  ...
  void _onPanEnd(DragEndDetails details) {
    ...
    double potentialX =
        _positionX + (details.velocity.pixelsPerSecond.dx * (widget.threshold ?? 1));
    double potentialY =
        _positionY + (details.velocity.pixelsPerSecond.dy * (widget.threshold ?? 1));
    ...
    bool swipedLeft = angle.abs() >= (3 * math.pi / 4) && angle.abs() <= (5 * math.pi / 4);
    //<135 deg 2 quad to >45 deg 1 quad
    bool swipedUp = angle.abs() < (3 * math.pi / 4) && angle.abs() > (math.pi / 4);
    //>225 deg 2 quad to <315 deg 1 quad
    bool swipedDown = angle.abs() > (5 * math.pi / 4) && angle.abs() < (7 * math.pi / 4);
  ...
  void _animate(double angle) {
    ...
    Velocity velocity = Velocity(
        pixelsPerSecond:
            Offset(remainingX / (widget.threshold ?? 1), remainingY / (widget.threshold ?? 1)));
